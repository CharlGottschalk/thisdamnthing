...
`tdt-software-production` is an optional stack in a separate source repository.
It coordinates work in linked software projects using core registration, skills
and UI. Its bundle guides define the project artifact formats.

## Install and inspect the workflow

```sh
tdt stack validate ../tdt-software-production
tdt --workspace ../workspace stack install ../tdt-software-production
tdt --workspace ../workspace stack docs tdt-software-production
```

Start a fresh host session to discover its skills. Use the full
`tdt-software-production-` prefix with `brief`, `init`, `add`, `task`, `build`,
...
Core UI can collect context, briefs and plans. Carry submitted answers into chat
...
Check both UI and chat entry paths for changed interviews. Verify external-provider
