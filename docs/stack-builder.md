...
`dryft-stack-builder` is an optional stack maintained in its own repository.
...
```sh
dryft stack validate ../dryft-stack-builder
dryft --workspace ../workspace stack install ../dryft-stack-builder
```

Start a fresh host session and invoke `dryft-stack-builder-create` through its
...
such as `dryft-example-greet`. List selected files explicitly in `stack.json`.
A Python file selected as an inert template must not become an executable hook.

The builder can gather missing details through core DUI or chat. Reuse submitted
...
Invoke `/dryft-stack-builder-publish` (Codex: `$dryft-stack-builder-publish`) with
...
Authors visit https://stacks.usedryft.com to submit the verified release for review;
...
[stack contract](../src/dryft/resources/harness/contracts/stack.md). Use core
[DUI](ui.md) and [capability](stack-capabilities.md) interfaces instead of adding
