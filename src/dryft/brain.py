...
def locked(root):
    read_config(root)
    path = managed_path(root, ".dryft/state/brain.lock")
...
def read_note(root, relative):
    ...
    if not text.startswith("---\n") or "\n---\n" not in text[4:]:
        raise WorkspaceError(f"Expected Dryft JSON front matter: {relative}")
...
def request_path(key):
    return f".dryft/state/captures/{identifier(key)}.json"
