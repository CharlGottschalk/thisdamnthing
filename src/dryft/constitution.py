...
POLICY = '.dryft/CONSTITUTION.md'
EXPECTED = '.dryft/state/constitution-expected'
...
def context(root):
    ...
    if policy['markdown'] is None:
        return 'Dryft workspace constitution: not configured. No additional workspace permissions are granted. Use /dryft-constitution to define rules.'
...
def save(root, text, expected, instruction):
    ...
    lock = policy_path(root, '.dryft/state/constitution-write.lock')
    try:
        lock.mkdir()
...
def request_hook(root):
    ...
    try:
        ...
        print(json.dumps({'decision': 'block', 'reason': f'Dryft policy could not load: {exc}'}))
        return 0
