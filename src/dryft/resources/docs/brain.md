...
Dryft turns useful facts, decisions and open questions from your conversations into
proposals for your brain. You decide which proposals become approved knowledge.
Use `/dryft-review-brain` to review them and `/dryft-search` to find what you know.

## Capture useful knowledge

With trusted capture hooks enabled, Dryft asks your active agent for a concise
...
Use `/dryft-review-brain` in Claude or `$dryft-review-brain` (or `/skills`) in Codex.
...
For technical users, `dryft brain candidates` shows pending proposals and their
exact review hashes. Direct review example (replace all placeholders with values
from the proposal):

```sh
dryft brain review ID --decision approve --expected-sha256 HASH --user-instruction 'User approved the displayed proposal'
```
...
Ask `/dryft-search` your question. It searches approved knowledge, follows bounded
links and answers with references, explaining missing or conflicting evidence.
In Codex, use `$dryft-search` or the skill picker. See [projects](projects.md)
...
remain for audit. `dryft brain search QUERY` searches approved canonical notes
...
If `dryft-search` is missing after an upgrade, ask your agent to refresh the
workspace (terminal: `dryft init <workspace>`) and
...
Use `/dryft-review-brain` once a recovered proposal is ready for review.
...
`dryft brain requests` lists incomplete capture requests. If the summary is malformed,
validation fails, or the host is interrupted, the request remains. Retry
`dryft brain capture ID` with a safe summary on stdin while the relevant context
...
protections coordinate Dryft processes, not arbitrary concurrent file editors.

## Optional offline search stacks

Literal search remains the default with no extra dependencies. Use
`/dryft-install-stack` to inspect and install a compatible local
`dryft-search-sqlite` or `dryft-search-semantic` bundle. Both work independently
and require approval of their exact executable digest.

Then use `/dryft-search`, naming the installed provider you want. Ask it to build
...
dryft brain index --provider dryft-search-semantic --rebuild
dryft brain search "How do we reduce release risk?" --provider dryft-search-semantic
dryft brain search "release risk" --provider dryft-search-sqlite --provider dryft-search-semantic
```
