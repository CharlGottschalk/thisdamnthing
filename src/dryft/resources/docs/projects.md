...
Use `/dryft-add-project` with the external project's path. The skill registers
its canonical directory and offers an explanation of its purpose and structure.
In Codex, use `$dryft-add-project` or the skill picker.

Terminal alternative: `dryft project add ../project` prints its stable ID and
...
Use `/dryft-add-project` in Claude or `$dryft-add-project` (or `/skills`) in Codex
...
`/dryft-review-brain` when satisfied.

For technical users, `dryft project propose ID` accepts the brain summary JSON
...
Ask `/dryft-add-project` to show registered projects or explain an existing
project using its current documentation.

Terminal alternatives: `dryft project list` reports IDs, canonical paths and
availability. `dryft project inspect ID` rereads bounded evidence. Missing/moved paths retain their identity;
