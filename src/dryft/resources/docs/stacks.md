...
format and examples, read `.dryft/contracts/stack.md`.
...
| Task | Skill |
| --- | --- |
| Find, inspect and install a local or marketplace stack | `/dryft-install-stack` |
| Inspect and apply a stack update | `/dryft-update-stack` |
| Remove an installed stack | `/dryft-remove-stack` |
| Check the workspace and locate installed stack guides | `/dryft-workspace` |
| Create and validate a new stack | `/dryft-stack-builder-create` (optional Stack Builder) |
| Prepare a stack release | `/dryft-stack-builder-publish` (optional Stack Builder) |
...
```sh
dryft stack validate ../stack
dryft --workspace . stack install ../stack
dryft --workspace . stack list
dryft --workspace . stack remove example-hello
```
...
trusted host (`dryft init . --agent both`). Skills have discovery
bridges for enabled hosts; restart the host to discover changes.

Use `/dryft-review-brain` to review imported knowledge. Installing a stack does not
approve its claims. Approved notes participate in `dryft brain search`; removing
...
run `dryft stack recover`, then retry; `dryft doctor` reports pending recovery or
changed stack files. Keep host sessions idle during these changes.

Skills use `dryft-*` names and follow the
[Agent Skills specification](https://agentskills.io/specification).
Stack IDs and repository folders use the same lowercase hyphen-separated name. Run `dryft init` on an existing
...
Use `/dryft-install-stack` to search a registry and inspect a selection before
...
```sh
dryft marketplace search "workflow" --agent codex
dryft marketplace search --browse popular
dryft stack install example-workflow --version 1.0.0 --inspect
dryft --workspace . stack install example-workflow --version 1.0.0
```
...
`/dryft-install-stack` guides the same inspection and installation commands.
Use an explicit `./directory` for a local directory whose name resembles an ID.

The default feed is `https://stacks.usedryft.com/registry/v1/index.json`.
...
Use `/dryft-workspace` to find the installed guides for the stack you want.
The skill reads the documentation catalog and explains available workflows.

Technical reference: `dryft stack docs` lists canonical local paths grouped by installed ID/version;
`dryft stack docs example-hello` limits the view to one stack. Neither command
opens a browser or uses the network. The guide index links to the generated
`.dryft/stack-docs.md` catalog. Only declared bundle docs are indexed, including
first-party and community guides; stacks with no docs are shown explicitly.
Docs remain in `.dryft/stacks/<id>/docs/` and do not become brain knowledge.

Install/update/remove transactions refresh the catalog and roll it back during recovery.
Updates refresh declared docs and remove obsolete owned guides. `dryft init` also builds the view
for older workspaces. `dryft stack docs --rebuild` repairs a missing or unchanged
...
`dryft stack docs --rebuild`. Keep this derived file unedited. Its companion
`.dryft/state/stack-docs.json` stores only an ownership digest; `stacks.json`
...
Use `/dryft-update-stack` to inspect and approve an update, or `/dryft-remove-stack`
...
```sh
dryft stack update example-hello --check
dryft stack update example-hello --approve APPROVAL_SHA256
dryft stack uninstall example-hello
```
...
Use `/dryft-install-stack`, `/dryft-update-stack` and `/dryft-remove-stack` for
capability bundles too, and `/dryft-search` to query an explicitly selected
...
To switch, use `/dryft-remove-stack` for the old ID, then `/dryft-install-stack`
for the normalized bundle after inspection. Terminal equivalents are
`dryft stack remove <old-id>` and `dryft stack install PATH`. Save any edited owned files first; removal
