...
Say “use dui” to request a local browser interview, or use `/dryft-ui`. You can
...
In Codex, use `$dryft-ui` or the skill picker. Describe the interview you want;
...
DUI directly or authoring a custom page.

```sh
dryft ui start --no-open
dryft ui present SESSION business.json
...
Read the full public contract at `.dryft/contracts/ui.md`. Resume after a crash
with `dryft ui start --session SESSION --no-open` and open the new URL. Read
retained answers with `dryft ui read SESSION --after 0`. A stopped or expired
...
`python`, `json` or `markdown`. DUI shows an inset monospace block, preserves
...
Custom pages use `<pre class="dui-code"><code></code></pre>` and set the code
