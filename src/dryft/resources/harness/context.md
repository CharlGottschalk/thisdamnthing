# Dryft workspace context

This is an installed Dryft knowledge workspace. The brain is in brain/;
.dryft/ is the canonical harness; docs/ contains usage guidance. Core requires
no stacks. Use /dryft-workspace for orientation and diagnostics.
...
bounded capture request, saving only pending candidates. /dryft-review-brain
...
`dryft brain search` retrieves only approved canonical notes. Use /dryft-search for cited linked answers and /dryft-add-project to link
external projects and offer onboarding. See docs/projects.md.

Optional local stacks: `dryft stack list` shows installed workflows and provenance.
Read .dryft/stack-docs.md for installed workflow guides and canonical bundle links;
`dryft stack docs [id]` lists their local paths. See docs/stacks.md and
.dryft/contracts/stack.md. Stack knowledge begins as pending
candidates; installation does not approve its claims.

Say “use dui” or invoke /dryft-ui for local browser questions, including custom
interactive pages. Honor an explicit DUI choice; chat/TUI remains available.
...
First check `dryft skill list` for matching core, stack and user skills and previous
...
Include a proposed dryft-name and what future invocations would do. Continue the
...
/dryft-find-skills N finds workflows repeated on two independent occasions in up to
20 completed previous workspace sessions, with honest history coverage limits.

Workspace permissions: use /dryft-constitution to define rules. Before every user
request load the current policy with `dryft constitution show` if a request hook
