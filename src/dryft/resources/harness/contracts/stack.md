...
| id | String of 1–80 lowercase letters, digits and single separating hyphens, starting with a letter; no dots, consecutive or trailing hyphens. Use the same ID for the repository and source directory, e.g. `dryft-search-sqlite`. |
...
| skills | Array of explicit `skills/dryft-<name>/SKILL.md` paths. |
...
Skills follow https://agentskills.io/specification. Names start with `dryft-`,
...
invocation as `/dryft-<name>` (Codex can use `$dryft-<name>`). Stack IDs and source
repository folders use the same lowercase hyphen-separated name, for example `dryft-software-production`.
...
```text
stack.json
skills/dryft-example-hello/SKILL.md
hooks/start.py
...
`dryft stack validate <directory>` displays manifest, canonical local origin,
...
Canonical files live under `.dryft/stacks/<id>/`. Skills are copied to owned
`.dryft/skills/` paths with thin `.agents/skills/` and `.claude/skills/` bridges.
Both namespaces are reserved even before host bootstrap. Directory/name/legacy
command conflicts fail preflight. Host naming is the existing `dryft-*` convention;
...
`.dryft/state/stacks.json` records manifest, origin, trust digest, candidate paths
...
`dryft stack recover` to restore the prior state. Recovery refuses intervening
...
`.dryft/stack-docs.md` catalog. Names come from file paths, not executable metadata
...
workspace-relative `.dryft/stacks/<id>/docs/...` references. Publishing readiness
...
- `compatibility` has exactly `dryft: "0.1"`, `platforms` (nonempty explicit list
...
it under `.dryft/state/capabilities/<id>/index`, with hash and snapshot ownership in
`.dryft/state/capability-state.json`. Failed/interrupted invocations discard temporary
