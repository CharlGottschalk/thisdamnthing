# Dryft UI contract v1

Public core interface for all stacks and agents; no stack manifest extension.
JSON commands use `dryft --workspace PATH ui …` outside the workspace. Errors
...
Inline `custom_html` can use `dui.submit(data, answers={})` or
`dui.action(name, data, answers={})`. Shared CSS variables and system font stacks
...
For code in custom pages, use `<pre class="dui-code"><code>…</code></pre>`
...
Each session has a private directory under `.dryft/state/ui/`, a JSON state file
...
and crashes. DUI does not write to the brain. An independently configured host
