---
name: dryft-add-project
description: Link an external project in place and offer evidence-based onboarding.
---

Find the Dryft workspace root. For the user's requested directory run
`dryft --workspace <root> project add <path>` with safely quoted arguments.
...
`dryft --workspace <root> project propose <id>`. Use title, kind (inference when
...
proposal and offer /dryft-review-brain for explicit review. Do not claim the
