---
name: dryft-constitution
description: Define or update a Dryft workspace constitution from natural-language permission preferences, with clarification and review. Use for workspace policy, not a linked project's constitution.
---

Read `docs/constitution.md` and run `dryft constitution show` from the workspace
...
default. If requested, use `/dryft-ui` and its retained answers for the interview.
...
`dryft.constitution.within(target, allowed, tree=True)` provides a boundary check
...
`dryft constitution save --expected-sha256 <shown-hash-or-missing> --user-instruction <brief-actual-approval-reference>`.
