...
const token = location.hash.slice(1) || sessionStorage.getItem('dui-token');
if (token) sessionStorage.setItem('dui-token', token);
...
function renderText(target,text){
  ...
  function block(){
    const pre=el('pre',null,{class:'dui-code',tabindex:'0','aria-label':language?language+' code':'Code block'});
...
function retain(){sessionStorage.setItem('dui-draft',JSON.stringify({round,answers,step}));}
...
function draw(){
  ...
  if(step<steps.length){
    ...
    for(const f of steps[step].fields){
      ...
      function change(v){answers[f.id]=v;pending=null;sessionStorage.removeItem('dui-pending');retain();}
...
async function send(action,data,customAnswers){
  ...
  sessionStorage.setItem('dui-pending',JSON.stringify(pending));
  try{await api('submit',pending);pending=null;sessionStorage.removeItem('dui-pending');currentStatus=action==='cancel'?'cancelled':'received';draw();}
  catch(e){message(e.message);if(e.fields){pending=null;sessionStorage.removeItem('dui-pending');const ids=Object.keys(e.fields);step=(page.steps||[]).findIndex(s=>s.fields.some(f=>ids.includes(f.id)));step=Math.max(0,step);draw();message(e.message);showErrors(e.fields);}else{button('Retry submission',()=>send(action,data,customAnswers));}}
  finally{busy=false;}
}
window.addEventListener('message',event=>{
  const d=event.data;
  if(!frame||event.source!==frame.contentWindow||event.origin!=='null'||!d||d.dui!==1||d.key!==bridgeKey||d.round!==round)return;
  ...
  try{
    ...
    if(result.page&&(result.round_id!==round||result.status!==currentStatus)){
      ...
      if(changed){answers=Object.create(null);step=0;pending=null;for(const s of page.steps||[])for(const f of s.fields)if(f.default!==undefined)answers[f.id]=f.default;
        try{const saved=JSON.parse(sessionStorage.getItem('dui-draft'));if(saved?.round===round){answers=saved.answers;step=saved.step;}const req=JSON.parse(sessionStorage.getItem('dui-pending'));if(req?.round_id===round)pending=req;}catch{}
      }
      draw();
