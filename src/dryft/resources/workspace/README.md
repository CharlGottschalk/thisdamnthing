...
Open your agent in this directory and use `/dryft-workspace` to inspect it.
In Codex, use `$dryft-workspace` or the skill picker. The skill runs diagnostics
and explains available workflows. Terminal alternative: `dryft doctor`.
Full core, stack and approved user skills live in `.dryft/skills/`.
...
use `dryft init [directory] --agent claude|codex|both|none` (choose one value).
