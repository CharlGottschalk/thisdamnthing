...
STATE = '.dryft/state/user-skills.json'
JOURNAL = '.dryft/state/user-skill-transaction.json'
MAX_PROPOSALS = 100


def name_checked(name):
    if (not isinstance(name, str) or len(name) > 64
            or not re.fullmatch(r'dryft-[a-z0-9]+(?:-[a-z0-9]+)*', name)
            or name in SKILLS):
        raise WorkspaceError('Expected an unreserved dryft-name, at most 64 characters')
    return name
...
def paths(name, root=None):
    ...
    selected = folders(root) if root is not None else ('.dryft/skills', '.claude/skills', '.agents/skills')
...
def render(proposal, bridge=False):
    ...
    body = (f"Read and follow .dryft/skills/{proposal['name']}/SKILL.md from the workspace root."
            if bridge else proposal['instructions'])
    return head + body + '\n'


def valid_record(record, name):
    return (f'.dryft/skills/{name}/SKILL.md' in record
...
def ready(root):
    ...
    if managed_path(root, STACK_JOURNAL).exists():
        raise WorkspaceError('Interrupted stack or agent operation; run dryft stack recover')
...
def inventory(root):
    ...
    results = []
    for folder in ('.dryft/skills', '.claude/skills', '.agents/skills'):
...
def recover(root):
    ...
    for relative, before in record['before'].items():
        if relative != STATE:
            match = re.fullmatch(r'(?:\.dryft|\.claude|\.agents)/skills/(dryft-[a-z0-9-]+)/SKILL.md', relative)
