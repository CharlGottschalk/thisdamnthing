...
INDEX = '.dryft/stack-docs.md'
OWNERSHIP = '.dryft/state/stack-docs.json'
REPAIR = ('Preserve/move .dryft/stack-docs.md elsewhere, then run '
          'dryft stack docs --rebuild. Never discard edits without reviewing them.')
...
def documents(root, entries, changes=None):
    ...
    for entry in sorted(entries, key=lambda e: e['id']):
        ...
        for doc in docs:
            safe_path(doc)
            relative = f".dryft/stacks/{entry['id']}/{doc}"
...
def diagnose(root, entries):
    ...
    if existing_text(root, INDEX) != expected or existing_text(root, OWNERSHIP) is None:
        raise WorkspaceError('Missing/stale stack documentation catalog; run dryft stack docs --rebuild')
