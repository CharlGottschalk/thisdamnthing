...
REGISTRY = '.dryft/state/stacks.json'
JOURNAL = '.dryft/state/stack-transaction.json'
...
def write_content(root, relative, value):
    ...
    fd, temporary = tempfile.mkstemp(prefix='.dryft-', dir=path.parent)
    try:
...
def validate(directory, *, legacy_skills=False, legacy_id=False, metadata_only=False):
    ...
    if not isinstance(data, dict) or not required <= set(data) or set(data) - required - ({'templates', 'docs', 'capabilities', 'assets', 'compatibility'} if data.get('contract_version') == 2 else {'templates', 'docs'}):
        raise WorkspaceError('Invalid stack fields; see .dryft/contracts/stack.md')
        ...
        for entry in entries:
            ...
            if category == 'skills':
                ...
                if (len(parts) != 3 or parts[2] != 'SKILL.md'
                        or not ((len(name) <= 64 and re.fullmatch(r'dryft-[a-z0-9]+(?:-[a-z0-9]+)*', name))
                                or (legacy_skills and re.fullmatch(r'dryft\.[a-z][a-z0-9-]*(?:\.[a-z][a-z0-9-]*)*', name)))
...
def registry(root):
    ...
    for entry in entries:
        ...
        for relative, digest in entry['files'].items():
            safe_path(relative)
            prefix = f".dryft/stacks/{entry['id']}/"
            if not (relative.startswith(prefix) or re.fullmatch(r'(?:\.dryft|\.agents|\.claude)/skills/dryft[.-][a-z0-9.-]+/SKILL.md', relative)):
...
def available(root):
    ...
    if managed_path(root, SKILL_JOURNAL).exists():
        raise WorkspaceError('Interrupted skill save; run dryft skill recover')
    if managed_path(root, JOURNAL).exists():
        raise WorkspaceError('Interrupted stack operation; run dryft stack recover')
...
def recover(root):
    ...
    bootstrap_paths = {*RESOURCES, *UI_RESOURCES, MANIFEST, UI_MANIFEST,
                       '.dryft/config.json', '.dryft/state/owned-files.json',
                       '.dryft/state/user-skills.json', '.dryft/state/capability-state.json', 'README.md',
                       *(p for provider in PROVIDERS.values() for p in (provider[0], provider[2]))}
    for relative, before in record['before'].items():
        safe_path(relative)
        if not (relative in bootstrap_paths or relative in (REGISTRY, '.dryft/stack-docs.md', '.dryft/state/stack-docs.json') or relative.startswith(('.dryft/stacks/', '.dryft/skills/', '.agents/skills/', '.claude/skills/', 'brain/candidates/', '.dryft/state/capabilities/'))):
    ...
    with locked(root):
        ...
        for relative in data['skills']:
            ...
            for folder in skill_folders:
                ...
                changes[f'{folder}/{name}/SKILL.md'] = files[relative] if folder == '.dryft/skills' else bridge(name)
...
def check_owned(root, entry):
    ...
    directories = {f".dryft/stacks/{entry['id']}"}
    directories.update(str(Path(p).parent) for p in entry['files']
                       if not p.startswith('.dryft/stacks/'))
...
def prune(root, files):
    boundaries = [root / p for p in ('.dryft/stacks', '.dryft/skills', '.agents/skills', '.claude/skills')]
...
def dispatch(root, event):
    ...
    for entry in available(root):
        ...
        for hook in manifest['hooks']:
            ...
            if remaining <= 0:
                print('dryft stack hook budget exhausted; remaining notifications skipped', file=sys.stderr)
                return
            ...
            try:
                ...
                print(f"dryft stack hook failed: {entry['id']} {hook['path']}: {exc}", file=sys.stderr)
...
def notify_hooks(root, event):
    try:
        ...
        print(f"dryft stack hooks unavailable: {exc}", file=sys.stderr)
