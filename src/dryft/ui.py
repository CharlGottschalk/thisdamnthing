...
def directory(root, sid):
    ...
    return managed_path(root, '.dryft/state/ui/' + sid)


def relative(sid, name):
    return f'.dryft/state/ui/{sid}/{name}'
...
def request(root, sid, route, payload=None):
    ...
    try:
        with ownership(root, sid):
            raise ValueError('Session disconnected; resume with dryft ui start --session ID')
    except SessionBusy:
        pass
    state = read(root, sid)
    require(type(state.get('port')) is int, 'Session disconnected; resume with dryft ui start --session ID')
    ...
    try:
        ...
        raise ValueError('Session disconnected; resume with dryft ui start --session ID') from exc
...
def start(root, sid=None, idle=1800, no_open=False):
    ...
    try:
        ...
        process = subprocess.Popen([sys.executable, '-m', 'dryft.ui', str(root), sid],
                ...
                try:
                    ...
                    if not self.path.startswith('/api/') and not post:
                        if self.path.startswith('/custom/'):
                            ...
                            html = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>' + resource_text('ui/tokens.css') + '</style><script>' + bridge + '</script></head><body class="dui-custom">' + state['page']['custom_html'] + '</body></html>'
                            self.respond(200, html, 'text/html', custom=True)
                            return
                        assets = {'/': ('shell.html', 'text/html'), '/shell.js': ('shell.js', 'text/javascript'), '/tokens.css': ('tokens.css', 'text/css'), '/wordmark.svg': ('wordmark.svg', 'image/svg+xml'), '/icon.png': ('icon.png', 'image/png')}
                        require(self.path in assets, 'Unknown asset')
                        name, mime = assets[self.path]
                        self.respond(200, files('dryft').joinpath('resources', 'ui', name).read_bytes(), mime)
                        return
