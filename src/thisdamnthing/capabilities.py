...
def validate_manifest(data, source, files, metadata_only=False):
    ...
    if (not isinstance(compat, dict) or set(compat) != {'tdt', 'platforms', 'python'}
            or compat['tdt'] != '0.1' or not isinstance(compat['platforms'], list)
        ...
        raise WorkspaceError('Invalid v2 compatibility (ThisDamnThing 0.1, explicit platform and Python minor lists)')
    assets = data['assets']
...
def selected(root, ids):
    ...
    for sid in sorted(ids):
        ...
        manifest, _, origin = stacks.validate(managed_path(root, f'.tdt/stacks/{sid}'), metadata_only=True, legacy_id=True)
...
def cache_path(sid):
    return f'.tdt/state/capabilities/{sid}/index'
...
def invoke(root, entry, documents, operation, query='', rebuild=False):
    ...
    with tempfile.TemporaryDirectory(prefix='tdt-provider-') as folder:
        ...
        base = managed_path(root, f'.tdt/stacks/{sid}')
