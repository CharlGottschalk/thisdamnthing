...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="tdt", description="A local knowledge workspace.")
    ...
    try:
        if args.command == "init":
            ...
            if created:
                ...
                print("Run tdt doctor to inspect its current state.")
            from .agents import enabled
            print("Enabled integrations: " + (", ".join(enabled(root)) or "none") + ". Run tdt doctor; review host trust before use.")
            return 0
        ...
        if args.command == "stack":
            ...
            if args.action == "install":
                ...
                for stack_id, version, paths in groups:
                    ...
                    for path in paths:
                        print(root / '.tdt/stacks' / stack_id / path)
                    if not paths:
        ...
        if args.command == "ui":
            ...
            print(f"tdt: {exc}", file=sys.stderr)
        return 1
