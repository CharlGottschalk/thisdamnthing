...
Ask `/tdt-find-skills 5` to look for repeated workflows in up to five completed
...
`tdt skill list` first: it includes core, stack, unmanaged and user skills in all
...
Submit a generalized proposal with `tdt skill propose` (JSON on stdin, at most
16 KiB). Use these fields:

```json
{
  "name": "tdt-unread-triage",
...
tdt skill review <id1> <id2> --decision approve --user-instruction <user-message-reference>
tdt skill review <id> --decision decline --user-instruction <user-message-reference>
```
...
`.tdt/state/user-skills.json`. Identical behavior has the same ID even when source
...
If interrupted, run `tdt skill recover`. It rolls back only files still matching
...
Claude uses `/tdt-find-skills 5` and `/tdt-unread-triage`. Codex uses
`$tdt-find-skills 5` and `$tdt-unread-triage` (slash selection depends on its UI).
...
```text
tdt skill history 5 --host claude --active-session <actual-id>
tdt skill history 5 --host codex --active-session <actual-id>
```
...
Approved skills always keep their full content in `.tdt/skills/`. Bridges are
