...
CONFIG = {"format_version": 1, "kind": "tdt-workspace", "adapters": {}}
STATE = {
    ".tdt/state/stacks.json": [],
    ".tdt/state/projects.json": [],
    ".tdt/state/owned-files.json": sorted([
        *TEMPLATES, ".tdt/config.json", ".tdt/state/stacks.json",
        ".tdt/state/projects.json", ".tdt/state/owned-files.json",
    ]),
}
...
def refuse_legacy_workspace(root):
    """Refuse unsupported harnesses without changing their contents."""
    legacy = root / ".dryft"
    if legacy.exists() or legacy.is_symlink():
        raise WorkspaceError(
            f"Unsupported workspace harness at {legacy}. Existing files are "
            "unchanged. Choose a separate empty directory for this workspace.")


def read_config(root):
    refuse_legacy_workspace(root)
    config = read_json(root, ".tdt/config.json")
    if (not isinstance(config, dict)
            or config.get("kind") != "tdt-workspace"
...
def resolve_workspace(explicit=None):
    ...
    for root in ([start] if explicit else [start, *start.parents]):
        refuse_legacy_workspace(root)
        if (root / ".tdt").exists() or (root / ".tdt").is_symlink():
            read_config(root)
            return root
    raise WorkspaceError("No ThisDamnThing workspace found. Run tdt init <directory> "
                         "or pass --workspace <path>.")


def resource_text(relative):
    return files("thisdamnthing").joinpath("resources", *relative.split("/")).read_text(
        encoding="utf-8")
...
def initialize(directory, agent=None):
    ...
    for parent in [root, *root.parents]:
        refuse_legacy_workspace(parent)
        if ((parent / ".dev/CONSTITUTION.md").is_file()
                and (parent / "docs/mvp.md").is_file()):
            raise WorkspaceError("This is the ThisDamnThing source repository. Choose a "
                                 "workspace directory outside it.")
        if parent != root and (parent / ".tdt").exists():
    ...
    config_path = managed_path(root, ".tdt/config.json")
    if config_path.exists():
        ...
        with locked(root):
            ...
            pending.update(plan_skills(root, json.loads(pending[".tdt/config.json"])["enabled_agents"]))
    ...
    if (root / ".tdt").exists():
        raise WorkspaceError(f"Conflicting harness at {root / '.tdt'}; "
    ...
    for relative in [*TEMPLATES, *STATE, ".tdt/config.json"]:
        path = managed_path(root, relative)
        if path.exists():
    ...
    content[".tdt/config.json"] = json.dumps(CONFIG, indent=2) + "\n"
    if agent is None:
    ...
    config_content = content.pop(".tdt/config.json")
    content[".tdt/config.json"] = config_content
    for relative in DIRECTORIES:
...
def diagnose(root):
    ...
    try:
        pending = ui_plan(root)
        if len(pending) > 1 or not managed_path(root, ".tdt/state/ui-resources.json").exists():
            raise WorkspaceError("Core UI resources need upgrade: rerun tdt init on this workspace")
